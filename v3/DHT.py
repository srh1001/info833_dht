...
class DHT(object):
    ...
    def join(self, joining_node: Node, target_node: Node):
        if joining_node.node_id in self.nodes.keys():
            ...
            print(f"[ {self.env.now} ] Node {joining_node.node_id} could not join DHT {self.name} as target not id {target_node.node_id} does not exist.\n")
    ...
    def join(self, joining_node_id: int, target_node_id: int):
        if joining_node_id in self.nodes.keys():
            ...
            print(f"[ {self.env.now} ] Node {joining_node_id} could not join DHT {self.name} as target not id {target_node_id} does not exist.\n")
